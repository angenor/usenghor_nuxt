...
const { t } = useI18n()
const { getProgramAlbums } = usePublicProgramsApi()
...
async function fetchAlbums() {
  loading.value = true
  try {
    const albumIds = await getProgramAlbums(props.programSlug)
    ...
    console.error('Erreur chargement médiathèque du programme:', e)
    albums.value = []
  }
  finally {
    loading.value = false
...
<template>
  <div class="py-8">
    <h2 class="text-2xl font-bold text-gray-900 dark:text-white mb-6 flex items-center gap-3">
      ...
      {{ t('formations.detail.tabs.mediatheque') }}
    </h2>
    ...
    >
      ...
      <p class="text-gray-600 dark:text-gray-400">{{ t('formations.detail.noMedia') }}</p>
    </div>
