...
const { getProgramNews } = usePublicProgramsApi()
...
async function fetchNews() {
  loading.value = true
  try {
    const data = await getProgramNews(props.programSlug)
    news.value = data as NewsPublicEnriched[]
  }
  catch (e) {
    console.error('Erreur chargement actualités du programme:', e)
    news.value = []
  }
  finally {
    loading.value = false
...
<template>
  <div class="py-8">
    <h2 class="text-2xl font-bold text-gray-900 dark:text-white mb-6 flex items-center gap-3">
      ...
      {{ t('formations.detail.tabs.actualites') }}
    </h2>
    ...
    >
      ...
      <p class="text-gray-600 dark:text-gray-400">{{ t('formations.detail.noNews') }}</p>
    </div>
    ...
    <div v-else class="grid gap-6 md:grid-cols-2">
      ...
      >
        ...
        <div class="h-48 overflow-hidden">
          ...
          >
            <font-awesome-icon icon="fa-solid fa-newspaper" class="w-16 h-16 text-gray-400 dark:text-gray-500" />
          </div>
        </div>
