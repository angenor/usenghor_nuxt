...
onMounted(() => {
  ...
  const project = getProjectByIdAdmin(projectId)

  if (project) {
    form.value = JSON.parse(JSON.stringify(project))
    // Charger le contenu EditorJS
    descriptionFr.value = project.description_fr as OutputData | undefined
    descriptionEn.value = project.description_en as OutputData | undefined
    descriptionAr.value = project.description_ar as OutputData | undefined
  } else {
    notFound.value = true
  }
...
const slugManuallyEdited = ref(true) // true car on édite un projet existant
...
const saveForm = async () => {
  ...
  try {
    ...
    // En production: PUT /api/admin/projects/{id}
    ...
    <template v-else>
      ...
      <div class="flex items-center justify-between">
        ...
        <div class="flex items-center gap-3">
          <span :class="['inline-flex rounded-full px-3 py-1 text-sm font-medium', projectStatusColors[form.status as ProjectStatus]]">
            {{ projectStatusLabels[form.status as ProjectStatus] }}
          </span>
          <button
      ...
      <div class="rounded-lg bg-gray-50 p-4 dark:bg-gray-900">
        <div class="flex flex-wrap gap-6 text-sm">
          ...
          <div>
            ...
            <span class="ml-1 text-gray-700 dark:text-gray-300">{{ formatDateTime(form.created_at) }}</span>
          </div>
          <div>
            <span class="text-gray-500 dark:text-gray-400">Modifié le :</span>
            <span class="ml-1 text-gray-700 dark:text-gray-300">{{ formatDateTime(form.updated_at) }}</span>
          </div>
        </div>
      </div>
