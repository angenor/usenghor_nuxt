...
import type { CampusTeamUser, CampusForSelect } from '~/composables/useCampusTeamApi'
...
const userSearchResults = ref<CampusTeamUser[]>([])
...
async function loadData() {
  ...
  try {
    campuses.value = await getCampusesForSelect()

    // Initialiser le campus depuis l'URL
    if (route.query.campus && typeof route.query.campus === 'string') {
      form.value.campus_id = route.query.campus
...
async function updateDisplayOrder() {
  if (form.value.campus_id) {
    try {
      const count = await countMembersByCampus(form.value.campus_id, false)
      form.value.display_order = count + 1
    }
    catch (error) {
      console.error('Erreur lors du comptage des membres:', error)
      form.value.display_order = 1
    }
  }
}

// Watch campus changes
watch(() => form.value.campus_id, async () => {
  await updateDisplayOrder()
})
...
function selectUser(user: CampusTeamUser) {
        ...
        <div>
          ...
          >
            ...
            <option v-for="campus in campuses" :key="campus.id" :value="campus.id">
              {{ getFlagEmoji(getCampusCountryCode(campus)) }} {{ campus.name }}
            </option>
          </select>
        </div>
        ...
        <div class="user-search-container relative">
          ...
          <div v-else class="relative">
            ...
            >
              ...
              >
                <font-awesome-icon :icon="['fas', 'spinner']" class="h-5 w-5 animate-spin text-gray-400" />
              </div>
            </div>
          </div>
