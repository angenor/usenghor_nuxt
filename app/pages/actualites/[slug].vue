...
const newsAlbums = ref<PublicAlbumWithMedia[]>([])
...
onMounted(async () => {
  ...
  if (news.value) {
    try {
      newsAlbums.value = await getNewsAlbums(news.value.slug)
    } catch { /* pas d'albums */ }
  }
})

const hasMediaLibrary = computed(() => newsAlbums.value.length > 0)
        ...
        <article class="lg:w-2/3">
          ...
          <div v-if="hasMediaLibrary" class="mb-8 border-b border-gray-200 dark:border-gray-700">
            <nav class="-mb-px flex gap-6">
              ...
              >
                {{ t('actualites.detail.news.tabs.details') }}
              </button>
              <button
                class="whitespace-nowrap border-b-2 pb-3 text-sm font-medium transition-colors"
              ...
              >
                ...
                {{ t('actualites.detail.news.tabs.mediatheque') }}
              </button>
            </nav>
          </div>
